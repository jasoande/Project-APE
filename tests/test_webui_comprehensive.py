...
def _check_api_endpoint(path, method="GET", data=None, expected_status=200, description="", csrf_token=None):
...
def test_page_rendering():
    ...
    for path, desc in pages:
        success, content = _check_api_endpoint(path, description=desc)
...
def test_api_endpoints():
    ...
    for path, method, data, expected, desc in endpoints:
        success, content = _check_api_endpoint(path, method, data, expected, desc)
        if success:
...
def test_configuration_api():
    ...
    success, content = _check_api_endpoint(
