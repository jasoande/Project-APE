# Project APE - Generic Container Image for Registry Distribution (Debian variant)
# Multi-stage build for optimized container size
#
# NOTE: This is the Debian/Ubuntu variant. For RHEL, use Containerfile (default)
...
LABEL description="Project APE - Account Planning Engine (Debian variant)"
LABEL version="2.0.0"
...
RUN apt-get update && apt-get install -y --no-install-recommends \
    # LibreOffice for document conversion (headless - install core-nogui FIRST)
...
COPY --chown=apeuser:apeuser container-entrypoint.sh /app/
