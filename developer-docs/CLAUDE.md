...
- **Deployment:** Podman containers (linux/x86_64, linux/arm64), registry: quay.io/jasoande/project_ape/project-ape
...
Project APE requires Google Drive API access for PDF source management and client data synchronization. OAuth2 authentication is handled via service credentials and user tokens.

**Quick Setup (Recommended):**
```bash
# Automated OAuth flow with credential validation
python3 setup-oauth-drive-improved.py

# The script will:
# 1. Guide you through creating OAuth credentials in Google Cloud Console
# 2. Automatically find the downloaded client_secret_*.json file
# 3. Move it to ~/.project-ape/drive_credentials.json
# 4. Set secure permissions (chmod 600)
# 5. Complete OAuth authentication flow
# 6. Save token to ~/.project-ape/drive_token.json
```
...
**Generated Files:**
- `drive_credentials.json` - OAuth client configuration (project root, gitignored)
  - Contains: client_id, client_secret, redirect_uris
  - Source: Google Cloud Console → APIs & Services → Credentials
  - Never commit to version control
- `token.json` - User access/refresh tokens (project root, auto-generated, gitignored)
  - Created after first successful OAuth flow
  - Auto-refreshes on expiry (90-day token lifetime)
  - Permissions: `chmod 600 token.json` (read/write for owner only)

**Security Best Practices:**
```bash
# Verify files are git-ignored
git check-ignore drive_credentials.json token.json  # Should return both paths

# Set restrictive permissions
chmod 600 drive_credentials.json token.json

# Container deployment: mount as read-only
podman run -v ./drive_credentials.json:/app/drive_credentials.json:ro,z \
           -v ./token.json:/app/token.json:ro,z project-ape:latest
```

**Token Lifecycle:**
- Access tokens: 1-hour expiry, automatically refreshed via `token.json`
- Refresh tokens: 90-day expiry (requires re-authentication after expiry)
- Revocation: tokens can be revoked via Google Account settings
- First auth opens browser for Google consent screen

**Troubleshooting:**
- **"Credentials not found":** Ensure `drive_credentials.json` exists in project root; run setup script
- **"Token has expired":** Delete `token.json`, re-run `setup-oauth-drive.py` to re-authenticate
- **"Access denied" / "Invalid scope":** Verify Drive API enabled in Google Cloud Console
- **"Redirect URI mismatch":** Check OAuth client type is "Desktop application" (not Web)
- **Container auth failures:** Mount both files to `/app/drive_credentials.json` and `/app/token.json` with `:z` SELinux label
- **Browser doesn't open:** Use `--manual` flag for console-based auth URL
...
- Project troubleshooting: `Docs/TROUBLESHOOTING.md` → "Google Drive Authentication"
...
**"OAuth setup hangs on project list":**
- Occurs when Google account has 100+ projects or sys-* projects
- Fixed in v4.0.1: project listing now filters sys-* projects, limits to 50 projects, 30s timeout
- If still hanging: delete `token.json` and re-run `setup-oauth-drive.py`
- Workaround: use `--manual` flag to skip project selection wizard
